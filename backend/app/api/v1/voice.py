...
@router.get("")
async def get_voice(db=Depends(get_db)):
    result = {}
    for mode in MODES:
        doc = await db.voice_profiles.find_one({"_id": mode})
        result[mode] = _to_response(doc)
    return result


@router.patch("/{mode}")
async def patch_voice_notes(mode: str, body: PatchNotesBody, db=Depends(get_db)):
...
async def rebuild_voice(mode: str, background_tasks: BackgroundTasks, db=Depends(get_db)):
    if mode not in MODES:
        raise HTTPException(status_code=400, detail="mode must be 'client' or 'internal'")
    doc = await db.voice_profiles.find_one_and_update(
        {"_id": mode},
    ...
    background_tasks.add_task(rebuild_profile, mode)
    return _to_response(doc)
